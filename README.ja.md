...
サンプルプログラムでは、デフォルトで同じ音素の類似度をオフセットとして使用し、実際の値から引いています。つまり、同じ音素の置換コストが0になるようにしています。これは、同じ音素の置換コストが0より大きいと、挿入コストが過大に見積もられ、編集距離の計算が直感に反することを防ぐためです。
...
### 母音や子音重みの影響の現れにくさ
サンプルプログラムでは、距離を計算する際に重みを設定できますが、音素がバイフォンで区別されるため、母音や子音の重みの影響はシンプルな編集距離を使用する場合に比べて、現れにくいです。
